            ...
            content: const SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 12),
                  Text(
                      'راهنما:', style: TextStyle(fontWeight: FontWeight.bold)),
                  SizedBox(height: 4),
                  ...
                  SizedBox(height: 12),
                  Text(
                      'با استفاده از تب‌ها، بین بخش‌های مختلف برنامه جابجا شوید.'),
                  SizedBox(height: 12),
                  Text(
      ...
      builder: (context) =>
          AlertDialog(
            shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(20)),
            title: const Text(
                'تولید داده‌های جدید', textAlign: TextAlign.center),
            content: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                ...
                onPressed: () {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                          '${min(max(places, 2),
                              10)} مکان گردشگری با موفقیت ایجاد شد!'),
                      backgroundColor: const Color(0xFF5B8CFF),
  ...
  Widget _buildDistancesTable(TouristPlannerModel model) {
    double cellWidth =
        (MediaQuery
            .of(context)
            .size
            .width - 32) / (model.places.length + 1);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'تعداد مکان‌های بازدید شده: ${model.optimalTour
                                .length}',
                            style: const TextStyle(
                              fontWeight: FontWeight.bold,
                              color: Color(0xFF5B8CFF),
                            ...
                            if (i > 0) ...[
                              ...
                              Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: Text(
                                      'زمان سفر از ${model.optimalTour[i -
                                          1]}: $travelTime دقیقه',
                                      style: TextStyle(
                                        color: Colors.grey.shade700,
                                        fontSize: 14,
...
class TouristPlannerModel extends ChangeNotifier {
  ...
  Map<String, dynamic> _dpSolve(String startPlace, int timeLimit) {
      ...
      for (int curr = 0; curr < n; curr++) {
        ...
        for (int next = 0; next < n; next++) {
          ...
          if (newTime <= timeLimit && newTime < dp[next][newMask]) {
            ...
            parent[next]
            [newMask] = List.from(parent[curr][mask])
              ..add(_places[curr]);
          }
        }
      }
    ...
    List<String> tour = [];
    if (lastPlace != -1) {
      tour = List.from(parent[lastPlace][finalMask])
        ..add(_places[lastPlace]);
    }
